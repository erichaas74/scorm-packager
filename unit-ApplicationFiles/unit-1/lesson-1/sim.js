...
const MotionGraphsLab = (function() {
 ...
 // Assessment Generators (Dynamically builds scenarios wrapping keyframes in entities arrays)
  const AssessmentGenerators = [
    ...
    { id:'q16', title:'BONUS Q16: Case 10 - Decoy Drop', bonus:true, generate: () => ({
      entities: [
        { id:'truck', isMain:true, color:'#2563eb', keyframes:[{t:0,x:0},{t:4,x:20},{t:7,x:20},{t:16,x:70}] },
        { id:'truck2', isMain:false, color:'#475569', keyframes:[{t:0,x:0},{t:3,x:40},{t:4,x:40},{t:8,x:60},{t:12,x:60},{t:16,x:0}] },
        { id:'suspectA', isMain:false, color:'#10b981', keyframes:[{t:0,x:0},{t:10,x:50},{t:16,x:80}] }
      ...
      duration:16,
      prompt:'BONUS: Case 10 shows two vans. Who steals the Gray Express van package after its delivery at House 6?',
      answer:'A', inputType:'choice', tolerance:0,
      choices:[{value:'A',label:'Suspect A'},{value:'B',label:'Suspect B'}],
      hint:'Follow the gray position line to its long stop at 60 m, then identify who reaches that location afterward.'
    })},
    { id:'q17', title:'BONUS Q17: Case 11 - Getaway Car', bonus:true, generate: () => ({
      entities: [
        { id:'truck', isMain:true, color:'#2563eb', keyframes:[{t:0,x:0},{t:3,x:30},{t:6,x:30},{t:10,x:70},{t:12,x:70}] },
        { id:'truck2', isMain:false, color:'#dc2626', keyframes:[{t:3,x:0},{t:5,x:30},{t:6,x:30},{t:8,x:70},{t:12,x:70}] }
      ],
      duration:12,
      prompt:'BONUS: In Case 11, at which house did the getaway car steal the package?',
      answer:'3', inputType:'house', tolerance:0,
      hint:'Find where the faster red line briefly stops at the same position where the blue delivery van stopped.'
    })},
    { id:'q18', title:'BONUS Q18: Case 12 - Hand-off Location', bonus:true, generate: () => ({
      entities: [
        { id:'truck', isMain:true, color:'#2563eb', keyframes:[{t:0,x:0},{t:6,x:40},{t:9,x:40},{t:15,x:70}] },
        { id:'truck2', isMain:false, color:'#475569', keyframes:[{t:0,x:70},{t:6,x:40},{t:9,x:40},{t:15,x:0}] },
        { id:'suspectA', isMain:false, color:'#10b981', keyframes:[{t:0,x:20},{t:6,x:50},{t:15,x:50}] },
      ...
      duration:15,
      prompt:'BONUS: In Case 12, at which house did the two vans meet for the package hand-off?',
      answer:'4', inputType:'house', tolerance:0,
      hint:'The hand-off occurs where the blue and gray position lines meet and remain flat together.'
    })},
    { id:'q19', title:'BONUS Q19: Case 12 - Identify the Thief', bonus:true, generate: () => ({
      entities: [
        { id:'truck', isMain:true, color:'#2563eb', keyframes:[{t:0,x:0},{t:6,x:40},{t:9,x:40},{t:15,x:70}] },
        { id:'truck2', isMain:false, color:'#475569', keyframes:[{t:0,x:70},{t:6,x:40},{t:9,x:40},{t:15,x:0}] },
        { id:'suspectA', isMain:false, color:'#10b981', keyframes:[{t:0,x:20},{t:6,x:50},{t:15,x:50}] },
      ...
      duration:15,
      prompt:'BONUS: In Case 12, who reaches the hand-off house after the vans leave?',
      answer:'B', inputType:'choice', tolerance:0,
      choices:[{value:'A',label:'Suspect A'},{value:'B',label:'Suspect B'}],
      hint:'Suspect A passes too early. Look for the suspect who reaches the hand-off position after t = 9 s.'
    })},
    { id:'q20', title:'BONUS Q20: Case 13 - Double Heist', bonus:true, generate: () => ({
      entities: [
        { id:'truck', isMain:true, color:'#2563eb', keyframes:[{t:0,x:0},{t:4,x:20},{t:7,x:20},{t:16,x:60}] },
        { id:'truck2', isMain:false, color:'#475569', keyframes:[{t:0,x:10},{t:4,x:50},{t:7,x:50},{t:12,x:70},{t:16,x:70}] },
        { id:'suspectA', isMain:false, color:'#10b981', keyframes:[{t:0,x:0},{t:12,x:40},{t:16,x:40}] },
      ...
      duration:16,
      prompt:'BONUS: In Case 13, who stole the Gray Express van package delivered at House 5?',
      answer:'B', inputType:'choice', tolerance:0,
      choices:[{value:'A',label:'Suspect A'},{value:'B',label:'Suspect B'}],
      hint:'Focus only on the gray line and its stop at 50 m. Then see which suspect reaches that position later.'
    })}
  ];
  ...
  const LOG_STORAGE_KEY = 'motion_graphs_bonus_assessment_v1';
  ...
  function coreAssessmentComplete() {
  ...
  function updateAssessmentProgress() {
    ...
    if (els.qSelect) {
      [...els.qSelect.options].forEach((option, i) => {
        const q = state.assessQuestions[i];
        const attempted = (state.logs[q.def.id] || []).length > 0;
        const correct = bestAssessmentScore(q.def.id) >= 100;
        option.textContent = (correct ? '✓ ' : attempted ? '↻ ' : '○ ') + q.def.title;
      });
      els.qSelect.value = String(state.assessIdx);
    }
    syncAssessmentScore();
  }
  ...
  function cacheDOM() {
    ...
    els.practicePlayBtn = document.getElementById('playBtn');
    els.practiceResetBtn = document.getElementById('resetBtn');
    els.assessPlayBtn = document.getElementById('assess-play-btn');
    els.assessResetBtn = document.getElementById('assess-reset-btn');
    els.playBtn = els.practicePlayBtn;
    els.resetBtn = els.practiceResetBtn;
    els.practiceStageControls = document.getElementById('practice-stage-controls');
    els.assessmentStageControls = document.getElementById('assessment-stage-controls');
    ...
    els.prevBtn = document.getElementById('prev-question');
    els.nextBtn = document.getElementById('next-question');
    els.statusSummary = document.getElementById('question-status-summary');
    els.topScore = document.getElementById('top-bar-score');
    els.overallScore = document.getElementById('overall-score-display');
  ...
  function init() {
    ...
    [els.practicePlayBtn, els.assessPlayBtn].filter(Boolean).forEach(btn => btn.addEventListener('click', togglePlay));
  ...
  function switchMode(mode) {
    ...
    if (els.practiceStageControls) els.practiceStageControls.classList.toggle('hidden', mode !== 'practice');
    if (els.assessmentStageControls) els.assessmentStageControls.classList.toggle('hidden', mode !== 'assessment');
    const directions = document.getElementById('assessmentDirectionsWrapper');
    const assessmentFeedback = document.getElementById('assessment-feedback-panel');
    if (directions) directions.classList.toggle('hidden', mode !== 'assessment');
    if (assessmentFeedback) assessmentFeedback.classList.toggle('hidden', mode !== 'assessment');
    if (els.feedbackWrapper) els.feedbackWrapper.classList.toggle('hidden', mode !== 'practice');
  ...
  function loadScenario() {
    ...
    if (state.mode === 'assessment') {
      ...
      if (els.assessFeedback) { els.assessFeedback.textContent = state.assessQuestions[state.assessIdx].def.bonus ? 'Optional bonus question' : ''; els.assessFeedback.className = 'feedback-summary'; }
      updateAssessmentProgress();
...
function setPlaybackLock(isLocked) {
  ...
  function submitAssessmentAnswer() {
    ...
    const currentQuestion = state.assessQuestions[state.assessIdx];
    const id = currentQuestion.def.id;
    if (!state.logs[id]) state.logs[id] = [];
    state.logs[id].push({ score: isCorrect ? 100 : 0, response: String(response), timestamp: Date.now() });
    saveAssessmentLogs();
    els.assessFeedback.textContent = isCorrect
      ? (currentQuestion.def.bonus ? 'Correct! Bonus credit added.' : 'Correct! 100%')
      : 'Not quite. The correct answer was ' + sc.answer;
  ...
  function checkPracticeAnswer(houseNum) {
    if (houseNum === state.scenario.deliveryHouse) {
      ...
      state.advanceTimer = setTimeout(() => { state.practiceIdx = Math.min(PracticeCases.length - 1, state.practiceIdx + 1); els.scenarioSelect.value = state.practiceIdx; loadScenario(); }, 1500);
    } else {
