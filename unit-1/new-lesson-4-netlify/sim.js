...
/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
  ...
  function getHostBridge() {
    return window.LabHostBridge || null;
  }

  function getSuspendDataRaw() {
    if (typeof SCORM !== 'undefined') return SCORM.getValue('cmi.suspend_data') || '';
    if (hostLaunchData && hostLaunchData.suspendData) return hostLaunchData.suspendData;
    return '';
  }

  function writeSuspendDataRaw(payload) {
    if (typeof SCORM !== 'undefined') {
      SCORM.setValue('cmi.suspend_data', payload);
      SCORM.commit();
      return true;
    }
    var bridge = getHostBridge();
    if (!bridge) return false;
    bridge.setSuspendData(payload);
    return true;
  }
  ...
  function clearPersistedProgress() {
    ...
    if (bridge) {
      bridge.clearState();
      bridge.setStatus('incomplete');
    }
    hostLaunchData = null;
  }

  function requestHostLaunchData() {
    var bridge = getHostBridge();
    if (!bridge || !bridge.requestLaunchData || !bridge.isEmbedded || !bridge.isEmbedded()) {
      return Promise.resolve(null);
    }
    return bridge.requestLaunchData().then(function(data) {
      hostLaunchData = data || null;
      return hostLaunchData;
    }, function() {
      hostLaunchData = null;
      return null;
    });
  ...
  function saveSuspendData() {
    try {
      ...
      writeSuspendDataRaw(payload);
    } catch (err) {}
  }

  function loadSuspendData() {
    var raw = getSuspendDataRaw();
    if (!raw) return false;
    try {
  ...
  function setSCORMProgress(finalize) {
    ...
    updateLessonStatus((finalize || state.submitted) ? 'completed' : 'incomplete');
  }
  ...
  function init() {
    requestHostLaunchData().then(function() {
      finishInit();
    });
  }
